...
def prepare_data(filename_SL, filename_random, IMG_SIZE, hdu = 0, keys = ['NAXIS', 'FILTER']):
    ...
    data_SL = np.array(data_SL)[:,:,:,0]
    #data_SL = data_SL[:,207:273,287:353]

    data_SL = resize(data_SL, (400,66,66))
    print(np.shape(data_SL))
    
    data = np.concatenate((data_SL, data_random), axis=0)
